...
| `file-utils` | `src/utils/file-utils.ts` | active | Read uploaded project metadata, build a folder path tree, and extract readonly TOML previews | `getProjectMetadata`, `readProjectArchive`, `analyzeProjectFiles` | Upload | Do not persist `File` or `FileList` |
